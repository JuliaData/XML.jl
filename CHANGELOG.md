...
- **`:strict` rejects a reference to an undeclared entity** ([#130](https://github.com/JuliaData/XML.jl/issues/130)): XML 1.0 §4.1's well-formedness constraint *Entity Declared* — one even a non-validating processor must enforce, where a validity constraint binds only a validating one. It binds only where the document carries every declaration that could name an entity — no DTD, or an internal subset alone with no parameter-entity reference and no external entity declared — since any other shape could declare the name beyond this reader's reach. Below `:strict` the reference stays literal, which is the behaviour every level had. This rejects eight more documents of the W3C not-well-formed suite, 408 of 1257.
