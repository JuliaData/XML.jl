...
- **A memory-mapped document is no longer copied into the heap when its lines end in CR** ([#135](https://github.com/JuliaData/XML.jl/issues/135)): the line-end normalization the specification requires used to rewrite the whole document at the entry, whatever string type it was held as — so mapping a file written on Windows asked for as much heap as the file, which is what mapping exists to avoid. A `String` document is still normalized once when it is read in; a document held as any other string type is left alone, and each value is normalized as it is reported, before entity resolution, so a `&#13;` still yields a real CR. On the 14 MB benchmark corpus, mapped, with CR LF line ends throughout, opening goes from 51.6 ms and 13.8 MiB to 13 ns and nothing, and opening then reading a thousand nodes from 57.1 ms to 50.8 µs; an all-LF file no longer pays a scan of the whole source either, 214 µs down to 12 ns. Documents held as a `String` — the common case — are unaffected in time and in allocation count alike, the normalization being selected by dispatch rather than by a run-time test. `sourcetext` reports the bytes of the document the reader holds, so a mapped document shows the file's own line ends where a rewritten `String` document shows normalized ones.
