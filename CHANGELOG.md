...
- **Internal general entities are included per XML 1.0 §4.4.2** ([#130](https://github.com/JuliaData/XML.jl/issues/130)): a reference to an entity declared in a DOCTYPE internal subset came through as literal text — `&e;` — in every reader. Its replacement text is included before the parse, so markup in that text produces structure rather than a text node holding `<`, and a reference in an attribute value is included in literal, its quotes re-escaped (§4.4.5). The first declaration of a name binds (§4.2); character references resolve at declaration time while entity references are bypassed (§4.5); declarations following a reference to an unread parameter entity are not used (§5.1). Expansion is bounded at depth 40 and 64 MiB, so a billion-laughs document raises an error rather than exhausting memory. Any document that uses internal entities changes the values it reports. A document read through a `StringView` over `Mmap` is covered by a package extension, so the recipe for files too large for the heap includes entities too, and a document that declares none costs one probe of its prolog, 18 ns on a 14 MB file. This turned 29 of the ledgered W3C canonical-reference gaps green and rejects four more not-well-formed documents.

- **`:strict` rejects a reference to an undeclared entity** ([#130](https://github.com/JuliaData/XML.jl/issues/130)): XML 1.0 §4.1's WFC: Entity Declared. The constraint binds only where the document carries every declaration that could name an entity — no DTD, or an internal subset alone with no parameter-entity reference and no external entity declared — since any other shape could declare the name beyond this reader's reach. Below `:strict` the reference stays literal, which is the behaviour every level had. This rejects eight more documents of the W3C not-well-formed suite, 408 of 1257.
